...
class RequestMailer < ApplicationMailer
  ...
  def new_response(info_request, incoming_message)
    ...
    mail(:from => contact_from_name_and_email,
         ...
         :subject => _("{{request_title}}", # New response to your FOI request - 
                       :request_title => info_request.title.html_safe),
         :charset => "UTF-8",
  ...
  def overdue_alert(info_request, user)
    ...
    mail_user(
      user,
      _("{{request_title}}", # Delayed response to your FOI request - 
        :request_title => info_request.title.html_safe)
    )
  end
  ...
  def very_overdue_alert(info_request, user)
    ...
    mail_user(
      user,
      _("{{request_title}}", # You're long overdue a response to your FOI request - 
        :request_title => info_request.title.html_safe)
    )
  end
  ...
  def not_clarified_alert(info_request, incoming_message)
    ...
    mail_user(
      info_request.user,
      _("{{request_title}}", # Clarify your FOI request - 
        :request_title => info_request.title.html_safe)
    )
  end
  ...
  def comment_on_alert(info_request, comment)
    ...
    mail_user(
      info_request.user,
      _("{{request_title}}", # Somebody added a note to your FOI request - 
        :request_title => info_request.title.html_safe)
    )
  end
  ...
  def comment_on_alert_plural(info_request, count, earliest_unalerted_comment)
    ...
    mail_user(
      info_request.user,
      _("{{request_title}}", # Some notes have been added to your FOI request - 
        :request_title => info_request.title.html_safe)
    )
  end
